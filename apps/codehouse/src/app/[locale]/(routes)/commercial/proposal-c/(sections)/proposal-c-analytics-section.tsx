...
export const ProposalCAnalyticsSection: FC<ComponentProps<'section'>> = ({ className, ...props }) => {
  const t = useTranslations('CommercialProposalCAnalyticsSection');
  const richText = useGlossaryRichText();

  return (
    <section className={cn('container max-w-screen-lg scroll-mt-16 py-4', className)} id="analytics" {...props}>
      <MotionDiv
          ...
          <MockWindow title={t('mock_title')}>
            <div className="space-y-3 p-3 sm:p-4">
              <div className="grid grid-cols-3 gap-2">
                <MockStat label={t('mock_stat_conversion')} value="4.8%" trend={t('mock_stat_conversion_trend')} />
                <MockStat label={t('mock_stat_retention')} value="91%" trend={t('mock_stat_retention_trend')} />
                <MockStat label={t('mock_stat_nps')} value="62" trend={t('mock_stat_nps_trend')} />
              </div>
